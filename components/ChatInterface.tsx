...
const RISK_MESSAGES = [
    "Perfecto, vamos a descubrir tu perfil de inversor. Te haré 5 preguntas rápidas. 🎯",
];
...
export default function ChatInterface() {
    ...
    // Auto-start risk test if redirected from dashboard with ?mode=test
    useEffect(() => {
    ...
    const handleRiskAnswer = async (answer: string) => {
        ...
        if (nextStep < 5) {
            ...
            setMessages(prev => [
                ...prev,
                { role: "assistant", content: `**Pregunta ${nextStep + 1} de 5:**\n${RISK_QUESTIONS_PREVIEW[nextStep]}\n\n*Responde con a), b) o c)*` },
            ]);
        } else {
            // All 5 answers collected — evaluate
            setIsLoading(true);
            setRiskMode(false);
            try {
                ...
                setMessages(prev => [
                    ...
                    {
                        role: "assistant",
                        content: `🎯 **Resultado del Test de Perfil**\n\n**Tu perfil: ${data.profile}**\n\n${data.explanation}\n\n📌 **Recomendaciones:**\n${data.recommendations}\n\n*Tu perfil ha sido actualizado. Ahora puedo darte consejos personalizados.*`,
                    },
                ]);
            } catch {
    ...
    const sendMessage = async () => {
        ...
        // Detect risk test trigger
        if (trimmed.toLowerCase().includes("test") || trimmed.toLowerCase().includes("perfil de riesgo")) {
            startRiskTest();
            return;
        }

        // In risk mode, treat input as answer
        if (riskMode) {
            handleRiskAnswer(trimmed);
            return;
        }
        ...
        try {
            ...
            setMessages(prev => [
                ...prev,
                {
                    role: "assistant",
                    content: data.reply,
                    marketData: data.market_data && !data.market_data.error ? data.market_data : undefined,
                },
            ]);
        } catch {
            setMessages(prev => [...prev, { role: "assistant", content: "❌ Error al conectar con el servidor. Asegúrate de que el backend está corriendo." }]);
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div style={{ display: "flex", flexDirection: "column", height: "100vh" }}>
            ...
            <header style={{
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <span className={`badge ${profileColors[profile]}`}>
                        Perfil: {profile}
                    </span>
                    {!riskMode && (
            ...
            {riskMode && (
                <div style={{
                    ...
                    <span style={{ fontSize: 12, color: "var(--accent)", fontWeight: 600 }}>
                        Test de Perfil — Pregunta {riskStep + 1} de 5
                    </span>
                    <div style={{ flex: 1, height: 4, background: "var(--border)", borderRadius: 2, overflow: "hidden" }}>
                        <div style={{
                            height: "100%",
                            width: `${((riskStep) / 5) * 100}%`,
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
                {messages.map((msg, idx) => (
                    <div key={idx} className="animate-fade-in" style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
                        {msg.role === "assistant" && (
                            <div style={{
                                width: 32, height: 32, borderRadius: 8, flexShrink: 0, marginRight: 10, marginTop: 2,
                        ...
                        <div style={{
                            ...
                            background: msg.role === "user"
                                ? "linear-gradient(135deg, var(--accent), #6b5ce7)"
                                : "var(--bg-card)",
                                ...
                                <div style={{
                                    ...
                                    <div style={{ textAlign: "right" }}>
                                        <div style={{ fontWeight: 700, fontSize: 16, fontVariantNumeric: "tabular-nums" }}>
                                            ${msg.marketData.price}
                                        </div>
                                        <div style={{
                                            fontSize: 12, fontWeight: 600,
                                            color: msg.marketData.change?.includes("-") ? "var(--red)" : "var(--green)",
                                        }}>
                                            {msg.marketData.change} ({msg.marketData.change_percent})
                                        </div>
                                    </div>
                                </div>
                            )}
                ...
                {isLoading && (
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <div style={{
                            width: 32, height: 32, borderRadius: 8, background: "linear-gradient(135deg, var(--accent), #7c5ce4)",
                            display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14,
                        }}>📈</div>
            ...
            <div style={{
                ...
                <div style={{ display: "flex", gap: 10, maxWidth: 800, margin: "0 auto" }}>
                    <input
                        ...
                        placeholder={riskMode ? "Responde con a), b) o c)..." : "Pregúntame sobre cualquier activo..."}
                        value={input}
                ...
                <p style={{ textAlign: "center", fontSize: 11, color: "var(--text-muted)", marginTop: 8 }}>
                    Solo uso educativo · No es asesoría financiera oficial
                </p>
            </div>
        </div>
    );
